...
no_data = np.loadtxt(os.path.join(root_dir, 'NO_correlation_dcp_t13.dat'))
io_data = np.loadtxt(os.path.join(root_dir, 'IO_correlation_dcp_t13.dat'))

theta13 = no_data[:, 0]
deltacp = no_data[:, 1]
chi2 = no_data[:, 2]

theta13_io = io_data[:, 0]
deltacp_io = io_data[:, 1]
chi2_io = io_data[:, 2] 

theta13_unique_no = np.unique(theta13)
deltacp_unique_no = np.unique(deltacp)
chi2_grid_no = chi2.reshape(len(theta13_unique_no), len(deltacp_unique_no))

theta13_unique_io = np.unique(theta13_io)
deltacp_unique_io = np.unique(deltacp_io)
chi2_grid_io = chi2_io.reshape(len(theta13_unique_io), len(deltacp_unique_io))


fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(18, 7))

levels_no = np.linspace(chi2.min(), chi2.min() + 50, 15)
contourf_no = ax1.contourf(deltacp_unique_no, theta13_unique_no, chi2_grid_no, 
                           levels=levels_no, cmap='viridis', extend='max')

contour_no = ax1.contour(deltacp_unique_no, theta13_unique_no, chi2_grid_no, 
...
confidence_contours_no = ax1.contour(deltacp_unique_no, theta13_unique_no, chi2_grid_no,
...
ax1.set_ylabel(r'$\theta_{13}$ [rad]', fontsize=12)
...
contourf_io = ax2.contourf(deltacp_unique_io, theta13_unique_io, chi2_grid_io, 
                           levels=levels_io, cmap='viridis', extend='max')

contour_io = ax2.contour(deltacp_unique_io, theta13_unique_io, chi2_grid_io, 
...
confidence_contours_io = ax2.contour(deltacp_unique_io, theta13_unique_io, chi2_grid_io,
...
ax2.set_ylabel(r'$\theta_{13}$ [rad]', fontsize=12)
ax2.set_title(r'Inverted Ordering (IO) - $\chi^2$', fontsize=14, fontweight='bold')
ax2.grid(True, alpha=0.3, linestyle='--', linewidth=0.5)

fig.suptitle(r'DUNE: Neutrino Oscillation using GLoBES - Correlation between $\delta_{cp}$ and $\theta_{13}$', fontsize=14, fontweight='bold', y=1.00)


plt.tight_layout()

output_filename = os.path.join(script_dir, 'correlation_dcp_t13_contour.png')
